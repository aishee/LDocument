...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        flash(u'Invalid user name or password error!', 'danger')
...
def logout():
    logout_user()
    flash(u'You have successfully signed out!', 'info')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Registration success! Welcome %s!' % form.name.data, 'success')
        login_user(the_user)
        return redirect(request.args.get('next') or url_for('main.index'))
