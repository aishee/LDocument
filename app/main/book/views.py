...
def edit(book_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash(u'Information books are saved!', 'success')
    ...
    return render_template("book_edit.html", form=form, book=book, title=u"Edit information books")
...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Books %s It has been added to library!' % new_book.title, 'success')
        return redirect(url_for('book.detail', book_id=new_book.id))
    return render_template("book_edit.html", form=form, title=u"Adding new books")
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Successfully deleted Books,users have been unable to see the books', 'info')
...
def put_back(book_id):
    ...
    db.session.add(the_book)
    db.session.commit()
    flash(u'Successful recovery books, users can now see the books', 'info')
    return redirect(request.args.get('next') or url_for('book.detail', book_id=book_id))
