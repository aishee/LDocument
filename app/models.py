...
class User(UserMixin, db.Model):
    ...
    def borrow_book(self, book):
        if self.logs.filter(Log.returned == 0, Log.return_timestamp < datetime.now()).count() > 0:
            return False, u"Unable to borrow,you have not returned overdue books"
        if self.borrowing(book):
            return False, u'Looks like you have to borrow this book!!'
        if not book.can_borrow():
            return False, u'This book is too fire,we have no holdings of,please wait for anyone else to borrow again return'

        db.session.add(Log(self, book))
        return True, u'GET your success to a %s' % book.title

    def return_book(self, log):
        if log.returned == 1 or log.user_id != self.id:
            return False, u'I did not find this record'
        log.returned = 1
